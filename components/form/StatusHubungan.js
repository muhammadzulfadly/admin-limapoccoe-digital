...
export default function Dusun({ value, onChange, error, disabled = false }) {
  return (
    <div className="relative">
      <label className="text-sm font-semibold text-gray-500">Status Hubungan<span className="text-red-500 ml-0.5">*</span></label>
      <select
        name="statushubungan"
        value={value}
        disabled={disabled}
        onChange={(e) => onChange({ name: "statushubungan", value: e.target.value })}
